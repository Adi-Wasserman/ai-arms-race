...
- **June 2026 hardening pass** (full review + fixes shipped):
  ...
  - **2029 Target Accountability tracker** (`src/features/race/TargetTracker.tsx` + `src/services/paceTracker.ts`): per-lab ON PACE/AHEAD/BEHIND/FAR BEHIND verdicts — required pace = (target − observed) ÷ quarters to Jan 2029 vs trailing ~180-day observed pace; thresholds 115/85/50%. **OpenAI's verdict is withheld ("BASIS GAP")** — its Azure fleet is invisible to the basis while the 15M target includes it; rendering a verdict would repeat the per-lab-undercount trap. Hover any pill for the full arithmetic. Resolves with each Epoch release.
  - **ChipConversionCard** (`src/features/race/ChipConversionCard.tsx`): spec-derived chip → H100e conversion reference (collapsed). Ratios are fixed by hardware specs (Trn2 0.93 = 918/989 BF16 TFLOPS, GB200 ≈2.5 sustained-training, Ironwood ≈2.3, Rubin provisional) — NOT user-adjustable; an interactive-sliders version was prototyped and rejected (the physics isn't an opinion). Uncertainty lives in fleet sizes/mixes, flagged by EST badges.
  - **HoverTip** (`src/components/ui/HoverTip.tsx`): instant styled tooltip (portal pattern, TipHeader/TipRow for structured content) replacing native `title` attributes on the EST/ADJ badges, INCL. ESTIMATES tags, key-assumption badges, and tracker pills. Native titles have an unfixable ~1s OS delay — use HoverTip for any new informational hover.
  - **Editorial pass (provenance + narrative)**: "key assumption" hover badges on the Colossus tenant legs in ComputeBreakdownCard (MW-share inference for COL-ANT, 1.0-H100e/GPU for COL-GGL); preview-model benchmark scores flagged at the cell level (amber asterisk + muted rank + legend) in BenchmarkTable; Race insight #4 = "The frontier is splintering" (differentiated from the Models takeaway); Race→Models bridge states the within-family proof + METR punchline; METR chart has a preamble tying it to the compute race; FirstPrinciples header = "biggest controllable lever"; GeoMap intro ties sites to 2029 targets + scope note on Chinese compute; OG/Twitter meta tags in index.html.
